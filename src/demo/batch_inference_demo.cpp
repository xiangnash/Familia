...
DEFINE_string(corpus_file, "corpus.txt", "corpus file");
...
int main(int argc, char* argv[]) {
    ...
    std::ifstream corpusFile(FLAGS_corpus_file);
    //std::ofstream corpusFile(FLAGS_result_file);
    
    while (getline(corpusFile, line)) {
        std::size_t pos = line.find("\t");
        string doc_id = line.substr(0, pos);
        vector<string> input;
        tokenizer->tokenize(line.substr(pos), input);
        cout << doc_id << " ";
